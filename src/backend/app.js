App = {
    ...
    init: () => {
        ...
        for(let i = firstNo; i <= lastNo; i++) {
            let formattedNo = i.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')
            ...
            numberTemplate.find('.panel-body .number').text(formattedNo)

            numberRow.append(numberTemplate.html())
        }
        // fit textsize of large numbers
        FileSystemEntry('#app .number', {
            minSize: 20,
            maxSize: 60
        })
    ...
    initContracts: () => {
      ...
      App.contracts.NumbersNFT.setProvider(window.web3.currentProvider)
        ...
        $.getJSON('NFTAuction.json', (contract) => {
            // instantiate truffle-contract with contract data
      App.contracts.NFTAuction = TruffleContract(contract)
      // set provider for contract
      App.contracts.NFTAuction.setProvider(window.web3.currentProvider)
        })
